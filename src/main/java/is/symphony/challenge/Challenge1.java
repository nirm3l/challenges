...
that they've brought with them, one item per line. Each Elf separates their own inventory from
...
carrying the most Calories. In the example above, this is 24000 (carried by the fourth Elf).
...
Calories carried by the top three Elves carrying the most Calories. That way, even if one of those Elves runs out of snacks,
...
public class Challenge1 {
    ...
    public int execute(final String filePath, final int sumOfTop) throws IOException {
        try (final Stream<String> lines = (Files.lines(Paths.get(filePath)))) {
            return lines
                    .map(line -> line.isBlank() ? -1 : Integer.parseInt(line))
                    .reduce(new ArrayList<>(List.of(0)), (list, value) -> {
                        ...
                        return list;
                            ...
                            .limit(sumOfTop).mapToInt(i -> i).sum();
        }
    }
}
